...
import { armorAnyStatHashes, armorStatHashes, statHashByName } from '../search-filter-values';
...
function calculateMaxPowerLoadoutItems(allItems: DimItem[]) {
  ...
  stores.forEach((store) => {
    _maxPowerLoadoutItems.push(...maxLightItemSet(stores, store).unrestricted.map((i) => i.id));
  });
}
